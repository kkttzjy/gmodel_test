...
power_sd = sqrt(power*(1-power)/nSIM)
pdf("Alternative case 1 100 simulation 99 bootstrap power plot w=seq(0,0.05,0.005).pdf")
plot(x = w, y = power, type = "l",
     xlab = "Change in combination rate", ylab = "Power",
     main = "Power plot")
arrows(w, power-power_sd, w, power+power_sd, angle=90,length=0.05,code=3,lwd=1)
text(w, power, power, adj = c(0.5, 1.5))
dev.off()
